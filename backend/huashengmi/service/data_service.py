...
class DataService(object):
    ...
    def jidu_salary_service(self, years):
        ...
        if len(years) > 1:
            return_result = {'category': {'years': years, 'seasons': ['第一季度', '第二季度', '第三季度', '第四季度']},
            ...
            for season in seasons:
                ...
                for year in years:
                    if year in sql_result.keys():
                        series[i].append(sql_result[year][season])
                    else:
                        series[i].append(0)
                i += 1
    ...
    def depart_and_month_service(self, months=[], depart=[], year=str(time.localtime()[0])):
        ...
        if len(months) > 0:
            depart_ID_list = config.depart_dict.keys()
            series = []
            depart_name_list = []
            depart_shortname_list = []
            for id in depart_ID_list:
                depart_shortname_list.append(config.depart_dict[id])
                depart_name_list.append(config.depart_name_dict[id])
            i = 0
            for sn in depart_shortname_list:
                series.append([])
                for month in months:
                    if month in sql_result.keys() and sn in sql_result[month].keys():
                        series[i].append(sql_result[month][sn])
                    else:
                        series[i].append(0)
                i += 1
            return_result = {'category' : {'month' : months, 'depart' : depart_name_list},
                             'series' : series,
    ...
    def depart_and_categary_service(self, year=str(time.localtime()[0]), month='01', depart=''):
        ...
        # 将数据库获取的数据按照接口标准重新组织
        sql_result = DataService.query_yangshi1_result(return_result[0], return_result[1])
        depart_name = config.depart_name_dict[depart]
        series = [0, 0, 0, 0, 0]
        if len(sql_result.values()) > 0:
            for dict in sql_result.values():
                for id in dict.keys():
                    if id in config.jixiao_category_dict:
                        series[config.jixiao_category_dict[id] - 1] += dict[id]
        max = 0
        for sum in series:
            if sum > max:
                max = sum
    ...
    def year_salary_for_download(self, year=str(time.localtime()[0])):
        return_result = self.dao.query_year_salary(year)
        sql_result = DataService.query_yangshi2_result(return_result[0], return_result[1])
        jixiao_ID_list = config.jixiao_dict.keys()
        jixiao_short_name_list = []
        jixiao_name_list = []
        for id in jixiao_ID_list:
            jixiao_short_name_list.append(config.jixiao_dict[id])
            jixiao_name_list.append(config.jixiao_name_dict[id])
    ...
    def query_yangshi1_result(sql_result, description):
        ...
        for row in sql_result:
            ...
            if row[1] in config.depart_dict:
                depart_short_name = config.depart_dict[row[1]]
                result_dict[row[0]][depart_short_name] = row[2]
            if row[1] in config.jixiao_dict:
    ...
    def query_yangshi2_result(sql_result, description):
        ...
        keys = [row[0] for row in description]
        for row in sql_result:
            if row[2] in config.depart_dict and row[3] in config.jixiao_dict:
                if int(row[0]) not in result_dict:
                    depart_name = config.depart_name_dict[row[2]]
                    result_dict[int(row[0])] = {keys[0]: row[0], keys[1]: row[1], keys[2]: depart_name, 'TOTAL' : 0}
                jixiao_short_name = config.jixiao_dict[row[3]]
